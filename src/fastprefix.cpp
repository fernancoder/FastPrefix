...
void use(const char *progName)
{
	...
	cout << "\t -c --configuration filename : Configuration file (default: fastprefix.ini)" << endl << endl;
	exit(0);
}
...
void verifyParams (int argc, char **argv)
{
	...
	const char* program_name = "fastprefix";
	int next_option;
...
int main (int argc, char **argv)
{
    ...
    if ( config_filename == NULL )
        strcpy(cfgFilePath, "./fastprefix.ini");
    else
    ...
    FastPrefixServer *server = new FastPrefixServer(cfgFilePath, appEnv, "main");
